...
EXPECTED_SHEETS = {
    ...
    DETAIL_SHEET: [["order_id"], ["date"], ["sku"], ["product_name", "name"], ["size"], ["price"], ["qty"], ["customer_name"]],
    MASTER_SHEET: [["order_id"], ["date"], ["items", "name"], ["total"], ["customer_name"]],
}
...
def save_order(order_id, data):
    ...
    detail.append_rows([row_for_headers(detail_headers, {"order_id": order_id, "date": order_date, "group": group, **item, "customer_name": customer_name}) for item in valid])
    master.append_row(row_for_headers(master_headers, {"order_id": order_id, "date": order_date, "group": group, "items": names, "name": names, "total": total, "customer_name": customer_name}))
    return {"order_id": order_id, "order_time": order_date, "customer_name": customer_name, "items": valid, "total": total}
...
def get_orders():
    ...
    for row in rows:
        ...
        if not oid:
            continue
        order = orders.setdefault(oid, {"order_id": oid, "date": clean_key(row, "date"), "group": clean_key(row, "group"), "customer_name": clean_key(row, "customer_name"), "items": [], "total": 0})
...
def update_order(order_id, data):
    ...
    date_index, customer_index = find_header_index(headers, "date"), find_header_index(headers, "customer_name")
    ...
    group = str(data.get("group", "")).strip()
    detail.append_rows([row_for_headers(headers, {"order_id": order_id, "date": original_date, "group": group, **item, "customer_name": customer_name}) for item in valid])
    ...
    row = row_for_headers(master_headers, {"order_id": order_id, "date": original_date, "group": group, "items": names, "name": names, "total": total, "customer_name": customer_name})
    ...
    return {"order_id": order_id, "date": original_date, "group": group, "customer_name": customer_name, "items": valid, "total": total}
